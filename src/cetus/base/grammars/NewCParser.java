...
public class NewCParser extends antlr.LLkParser implements NEWCTokenTypes {
	...
	public void putPragma(Token sline, SymbolTable sym) {
		...
		for (int i = 0; i < vsize; i++) {
			...
			anote.line_in_original_file = sline.getLine();
			anote.column_in_original_file = sline.getColumn();
			...
			if (sym instanceof CompoundStatement) {
				...
				((CompoundStatement) sym).line_in_original_file = sline.getLine();
				((CompoundStatement) sym).column_in_original_file = sline.getColumn();
			}
	...
	public final Specifier typedefName() throws RecognitionException, TokenStreamException {
		...
		try { // for error handling
			...
			if (inputState.guessing == 0) {
				...
				((UserSpecifier) name).getIDExpression().line_in_original_file = i.getLine();
				((UserSpecifier) name).getIDExpression().column_in_original_file = i.getColumn();
	...
	public final Declarator enumerator() throws RecognitionException, TokenStreamException {
		...
		try { // for error handling
			...
			if (inputState.guessing == 0) {
				...
				decl.column_in_original_file = i.getColumn();
				decl.line_in_original_file = i.getLine();
		...
		try { // for error handling
			...
			if (inputState.guessing == 0) {
				...
				if (binit instanceof Expression && decl != null && binit != null && !extern_decl) {
					...
					// initial_expr = new AssignmentExpression(lhsID , AssignmentOperator.NORMAL,
					// (Expression)binit);
	...
	public final List idList() throws RecognitionException, TokenStreamException {
		...
		try { // for error handling
			...
			if (inputState.guessing == 0) {
				...
				name = idl1.getText();
				VariableDeclaration v = new VariableDeclaration(new VariableDeclarator(new NameID(name)));
				v.line_in_original_file = idl1.getLine();
				v.column_in_original_file = idl1.getColumn();
				ilist.add(v);
			...
			switch (LA(1)) {
				...
				case DOT: {
					...
					if (inputState.guessing == 0) {
						...
						designator.line_in_original_file = id.getLine();
						designator.column_in_original_file = id.getColumn();
						if (expr3 != null) {
							ret = new Initializer(designator, expr3);
						...
						ret.line_in_original_file = id.getLine();
						ret.column_in_original_file = id.getColumn();
	...
	public final CompoundStatement compoundStatement() throws RecognitionException, TokenStreamException {
		...
		try { // for error handling
			...
			if (inputState.guessing == 0) {
				...
				stmt.line_in_original_file=lcur.getLine();
				stmt.column_in_original_file=lcur.getColumn();
				enterSymtab(stmt);
					...
					switch (LA(1)) {
						case PTR: {
							...
							if (inputState.guessing == 0) {
								...
								ret_expr = new AccessExpression(
										ret_expr, AccessOperator.POINTER_ACCESS,
										SymbolTools.getOrphanID(ptr_id.getText()));
						...
						case DOT: {
							...
							if (inputState.guessing == 0) {
								...
								ret_expr = new AccessExpression(
										ret_expr, AccessOperator.MEMBER_ACCESS,
										SymbolTools.getOrphanID(dot_id.getText()));
	...
	public final void strOptExprPair(
			List<Traversable> expr_list) throws RecognitionException, TokenStreamException {
