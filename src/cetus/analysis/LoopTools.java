...
public class LoopTools {
    ...
    public static LinkedList<Loop> getOutermostLoops(Traversable traversable) {

        LinkedList<Loop> loops = getAllLoops(traversable);
        for (int i = loops.size() - 1; i >= 0; i--) {
            if (!LoopTools.isOutermostLoop((ForLoop) loops.get(i))) {
                loops.remove(i);
            }
        }
    ...
    private static LinkedList<Loop> getAllLoops(Traversable traversable) {
        ...
        DepthFirstIterator<Traversable> depthFirstIterator = new DepthFirstIterator<>(traversable);
        while (depthFirstIterator.hasNext()) {
            Traversable auxTraversable = depthFirstIterator.next();
            if (auxTraversable instanceof Loop) {
                loops.add((Loop) auxTraversable);
            }
        }
        return loops;
    }
