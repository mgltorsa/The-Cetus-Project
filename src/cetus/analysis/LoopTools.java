...
public class LoopTools {
    ...
    /**
     * Verify if the increment is part of the tiling pass
     */
    public static boolean isTileIncrement(Loop loop) {
        if(!(loop instanceof ForLoop)) {
            return false;
        }
        ForLoop for_loop = (ForLoop) loop;
        Expression inc = for_loop.getStep();

        if (inc instanceof IntegerLiteral) {
            return true;
        }
        
        if(!(inc instanceof AssignmentExpression)) {
            return false;
        }
        AssignmentExpression assign_exp = (AssignmentExpression) inc;
        inc = assign_exp.getRHS();
        ...
        return incStr.startsWith(Tiler.IN_TILE_PREFIX); 
    }
    ...
    /**
     * Check if the input access is marked as private to the loop by the
     * Privatization pass. ArrayPrivatization MUST be run before using this
     * test.
     * 
     * @param arrayAccess the array Access which needs to be checked as private to
     *                    loop or not
     * @param l           the loop with respect to which private property needs to
     *                    be
     *                    checked
     * @return true if private, else false
     */
    public static boolean isPrivate(ArrayAccess arrayAccess, Loop l) {
        if (arrayAccess.getIndices().size() == 0) {
            return false;
        }
        Expression firstIndexExpr = arrayAccess.getIndex(0);
        Symbol accessSymbol = SymbolTools.getSymbolOf(firstIndexExpr);
        if (accessSymbol == null) {
            return false;
        }
        return isPrivate(accessSymbol, l);
    }
     ...
     */
    private boolean isParallelizableLoopNest(Loop enclosing_loop, DDGraph dependence_graph) {
        ...
        for (int i = 0; i < eligible_loops.size(); i++) {
            ...
            for (int j = 0; j < contained_nest.size(); j++) {
                ...
                if (is_parallel) {
                    ...
                    if (!has_scheduled_outer_loop && Driver.isIncluded("parallelize-loops",
                            "Loop", LoopTools.getLoopName((Statement) l))) {

                        CetusAnnotation note = new CetusAnnotation();
                        note.put("parallel", "true");
                        ((Annotatable) l).annotate(note);

                        try {
                            int numOfProcessors = Integer
                                    .parseInt(Driver.getOptionValue(TilingParams.CORES_PARAM_NAME));

                            CetusAnnotation numThreadsAnnot = new CetusAnnotation();
                            numThreadsAnnot.put("num-threads", numOfProcessors);
                            ((Annotatable) l).annotate(numThreadsAnnot);
     ...
     * Reduction pass. Reduction Analysis MUST be run before using this test
     * 
     * @param access the arrayAccess which needs to be checked as reduction variable
     *               or not
     * @param l      the loop with respect to which reduction property needs to be
     *               checked
     * @return true if reduction variable, else false
     */
    public static boolean isReduction(ArrayAccess access, Loop l) {
        if (access.getIndices().size() == 0) {
            return false;
        }
        Expression firstIndexExpr = access.getIndex(0);
        Symbol accessSymbol = SymbolTools.getSymbolOf(firstIndexExpr);
        if (accessSymbol == null) {
            return false;
        }
        return isReduction(accessSymbol, l);
    }
