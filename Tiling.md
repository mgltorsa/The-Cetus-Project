# Parallel-Aware Tiling (PAW Tiling) Documentation
...
- [How to Run](#how-to-run)
  ...
  - [Command Line Options](#command-line-options)
    - [Main Tiling Option](#main-tiling-option)
...
## FAQs
- [How to Run?](#how-to-run)
- What was updated? *See: [Integration with Cetus Passes](#integration-with-cetus-passes) and [Troubleshooting](#troubleshooting).
- Which are some running examples?: *See examples [here](#running-examples)*.
- How is legality performed? *See [Legality](#legality-check)*
- Which snippets outperforms? *Pending*
...
- **Serial Tiling for testing**: Expose a way to do serial tiling by disabling parallelization and disabling certain passes that can interfere with serial tiling. 
...
- **Integration**: This passes integrates with existing Cetus analysis and transformation passes.
...
```bash
# Compare different algorithms on same benchmark
...
5. **Tile Size Algorithms** - LRW, NT, and Fixed algorithms
...
Since PAW Tiling can be a complex pass that 
...
- **Loop Tools**: Detecting tile increments that were not parallelized due to an required symbolic validation on the increment variable (Lines: 1188 and 1190).
...
- `block_factor` = total of data structures. Usually it uses a factor of 3 accounts for typical matrix operations (A, B, C matrices).
...
9. **`Driver.java`** - Main driver class
   ...
   - **Key Integration Points**: Lines 357-405 (option registration), Lines 897-899 (pass execution)

10. **`LoopParallelizationPass.java`**
    - **Modifications**:
      - Fixed a bug on lines: 274-278. The bug was related to not properly identifying private and reduction variables.
     ```java
...
11. **`LoopTools.java`**
    - **Modifications**
      - Added a way to detect tile increments (when increments are complex expressions or variables references):
      ```java
...
**Solutions**:
- Increase `-cacheSize` parameter
- Try different `-selection` algorithm
...
1. **Documentation**: Refer to Cetus user manual
2. **Debug Output**: Use `-verbosity=4` for detailed information
3. **Community**: Check Cetus project website
4. **Issues**: Report bugs through project issue tracker
